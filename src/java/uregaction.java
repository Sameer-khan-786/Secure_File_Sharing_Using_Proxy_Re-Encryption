 ...
 * @author ADMIN
 */
@WebServlet(urlPatterns = {"/uregaction"})
public class uregaction extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
       ...
       List<String> ms = new ArrayList<String>();
		...
		{   
                    ...
                    PreparedStatement ps=con.prepareStatement("insert into ureg(profile,id,user,pass,email,dob,gender,mobile,location)values('"+ms.get(0)+"','"+m.get(0)+"','"+m.get(1)+"','"+m.get(2)+"','"+m.get(3)+"','"+m.get(4)+"','"+m.get(5)+"','"+m.get(6)+"','"+m.get(7)+"')");
                    System.out.println(ps);
                    ps.executeUpdate();
                    System.out.println(ps);
                    out.println("<script>"); 			
                    out.println("alert(\"Data User Register Succesfully\")");
                    out.println("</script>");
